...
func (t *progressTracker) newSharedPullerState(file protocol.FileInfo, folder, tempName, realName string, blocks, reused int, ignorePerms bool, availableBlocks []protocol.BlockInfo) *sharedPullerState {
	s := &sharedPullerState{
		progressTracker: t,
		file:            file,
		folder:          folder,
...
func (t *progressTracker) finished(s *sharedPullerState) {
	t.mut.Lock()
	defer t.mut.Unlock()
	for i, state := range t.activePullers {
		if state.folder == s.folder && state.file.Name == s.file.Name {
			t.lastChange = time.Now()
			last := len(t.activePullers) - 1

			t.activePullers[i] = t.activePullers[last]
			t.activePullers[last] = nil
			t.activePullers = t.activePullers[:last]
